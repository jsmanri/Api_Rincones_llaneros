...
type Comentarios struct {
	Id                 int               `orm:"column(Id_Comentario);pk;auto"`
	IdSitiosTuristicos *SitiosTuristicos `orm:"column(Id_Sitios_turisticos);rel(fk)"`
	IdUsuario          *Usuarios         `orm:"column(Id_Usuario);rel(fk)"`
	Comentario         string            `orm:"column(Comentario)"`
	Calificacion       string            `orm:"column(Calificacion)"`
...
func GetAllComentarios(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Comentarios)).RelatedSel()
	// query k=v
	for k, v := range query {
